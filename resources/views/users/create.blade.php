...
@section('titulo')- Cadastrar Funcionário @endsection
...
@section('conteudo')
    ...
    <form action="{{ route('users.store') }}" method="POST">
                ...
                <div class="form-floating mb-3">
                    ...
                    <option value="{{null}}">SELECIONE A FUNÇÃO</option>
                        @foreach ($roles as $item) 
                            <option value="{{$item->id}}"  @if($item->id == old('role_id')) selected="true" @endif>
                                {{$item->nome}}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="row">
            <div class="col">
                <a href="{{route('users.index')}}" class="btn btn-secondary btn-block align-content-center">
